      ...
      <tr>
         ...
         <td>
            <center><a href="/impacts" class="btn btn-sm">
               ...
               <span title="Number of not acknownledged IT problems." class="badger-big background-{{'ok' if overall_state == 0 else 'warning' if overall_state == 1 else 'critical'}}">{{len(app.datamgr.get_impacts(user, search='is:impact bi:>=0 type:all isnot:ACK isnot:DOWNTIME'))}}</span>
            </a></center>
         </td>
      </tr>
   </tbody>
